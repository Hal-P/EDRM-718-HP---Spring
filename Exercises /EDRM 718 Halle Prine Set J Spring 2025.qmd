...
# Factor SES if not already
mental$SES <- factor(mental$SES, levels = c(1, 2, 3), labels = c("Low", "Medium", "High"))

# Create the plot
ggplot(notes_dta, aes(x = notes, y = score, color = notes)) +
  geom_point(aes(shape = lecture), alpha = 0.6) +  # Plot points with different shapes for LectureType
  geom_smooth(method = "lm", se = TRUE, linetype = "solid") +  # Add regression line without interaction
  labs(title = "Effect of Note-Taking Method on Recall",
       x = "Note-Taking Method",
       y = "Recall Score",
       color = "Note-Taking Method",
       shape = "Lecture Type") +
  theme_minimal() +
...
# Factor the lecture and note-taking method columns for better plotting
notes_dta$lecture <- factor(notes_dta$lecture, levels = c(1, 2), labels = c("Disorganized", "Organized"))
notes_dta$notes <- factor(notes_dta$notes, levels = c(1, 2, 3), labels = c("Outline", "Linear", "Matrix"))

ggplot(notes_dta, aes(x = score, y = factor(notes), color = factor(notes))) +
  geom_point(aes(shape = factor(lecture)), alpha = 0.6) +  # Convert lecture to a factor
  geom_smooth(aes(group = factor(notes)), method = "lm", se = TRUE, linetype = "solid") +  # Add regression lines
  labs(title = "Effect of Note-Taking Method on Recall, Moderated by Lecture Type",
       x = "Recall Score",
       y = "Note-Taking Method",
       color = "Note-Taking Method",
       shape = "Lecture Type") +
  theme_minimal() +
  theme(legend.position = "top")  # Adjust legend position
